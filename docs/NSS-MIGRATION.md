...
拆分缓存缺失时按旧版 `dl` + `.ccache` 路径尝试迁移，已有新缓存优先保留；构建串行且不取消正在运行的任务。工具链按输入键保存，下载内容变化时保存，ccache 保留本地 `2G` 上限。

未获授权自动清理远端缓存，不新增远端删除操作或付费扩容。`2G` 不是仓库最优总预算或容量保证；总占用需按实际压缩归档评估，不以未压缩 tar 大小拦截正常工具链上传，也不为估算重复打包。保持原生 `actions/cache` 格式；容量与保留策略仍待单独审查和授权。
