...
class DownloadCacheTests(unittest.TestCase):
    def test_workflow_gates_final_contents_and_cache_miss(self):
        ...
        self.assertIn("steps.downloads.outputs.cache-matched-key == '' || steps.cache-selection.outputs.legacy-downloads == 'true' || steps.downloads-before.outputs.digest != steps.downloads-after.outputs.digest", workflow)
