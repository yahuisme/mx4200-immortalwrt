...
class LegacyCacheTests(unittest.TestCase):
    def test_restore_matrix(self):
        ...
        self.assertIn("steps.downloads.outputs.cache-matched-key == ''", legacy['if'])
        self.assertIn("steps.ccache.outputs.cache-matched-key == ''", legacy['if'])
        self.assertIn(' || ', legacy['if'])
        ...
        self.assertLess(steps.index(recover), steps.index(named['Restore toolchain']))
        for dl_hit, cc_hit, legacy_hit in itertools.product((False, True), repeat=3):
            with self.subTest(dl=dl_hit, cc=cc_hit, legacy=legacy_hit), tempfile.TemporaryDirectory() as directory:
                ...
                env = dict(os.environ, DOWNLOADS_MATCHED='new-key' if dl_hit else '',
    ...
    def test_failed_legacy_extraction_recovers_hit_and_shell_errors_propagate(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            env = dict(os.environ, DOWNLOADS_MATCHED='prefix-hit', CCACHE_MATCHED='')
            def run(name):
